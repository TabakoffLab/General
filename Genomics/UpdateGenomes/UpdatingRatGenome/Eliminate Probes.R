rm(list=ls())
setwd("~/Documents/Affymetrix/SNP Masks/Exon Array/Rat")

pgfFile <- read.table(file="newpgffile.pgf",header=TRUE,sep="\t",fill=TRUE)
## 4,104,557 probes

uniqueHits <- read.table(file="Source/snpsInUniqueProbes.txt",sep="\t",header=FALSE)
...
## 3,551,816 probes uniquely hit the genome
## 552,741 probes did NOT uniquely hit the genome


probeStatus$status[probeStatus$V5>0 & probeStatus$status=="unique Hit"] <- "with snp"
## 132,400 uniquelly mapping probes with known SNP

table(probeStatus$status)

#4,104,557 original probes

# 552,741 did not hit the genome or hit multiple places in the genome
# 132,566 probes contain a SNP between SHR and BN
#__________________________________
# 685,307 probes to be removed
...
# 604,601 of probes to be removed are "main" probes
...
core.ps <- read.table(file="Source/RaEx-1_0-st-v1.r2.dt1.rn4.core.ps",sep="\t",comment.char="")
...
#13,920 core probe sets removed

write.table(reduced.core.ps,file="Results/RaEx-1_0-st-v1.r2.dt1.rn4.core.MASKED.ps",sep="\t",col.names=FALSE,row.names=FALSE,quote=FALSE)

extended.ps <- read.table(file="Source/RaEx-1_0-st-v1.r2.dt1.rn4.extended.ps",sep="\t",comment.char="")
...
#34,955 core/extended probe sets removed

write.table(reduced.extended.ps,file="Results/RaEx-1_0-st-v1.r2.dt1.rn4.extended.MASKED.ps",sep="\t",col.names=FALSE,row.names=FALSE,quote=FALSE)

full.ps <- read.table(file="Source/RaEx-1_0-st-v1.r2.dt1.rn4.full.ps",sep="\t",comment.char="")
...
#166,411 full probe sets removed

write.table(reduced.full.ps,file="Results/RaEx-1_0-st-v1.r2.dt1.rn4.full.MASKED.ps",sep="\t",col.names=FALSE,row.names=FALSE,quote=FALSE)

reduced.all.ps <- c(as.character(full.ps[1:12,]),as.character(retainProbeSet))
write.table(reduced.all.ps,file="Results/RaEx-1_0-st-v1.r2.dt1.rn4.all.MASKED.ps",sep="\t",col.names=FALSE,row.names=FALSE,quote=FALSE)
...
write.table(forPython,file="Results/core.mps",quote=FALSE,sep="\t",na="0",row.names=FALSE,col.names=TRUE)

###  Extended Transcript Clusters  ###

extended.mps <- read.table(file="Source/RaEx-1_0-st-v1.r2.dt1.rn4.extended.mps",sep="\t",header=TRUE)
...
forPython <- forPython[order(forPython[,"transcript_cluster_id"]),]
write.table(forPython,file="Results/extended.mps",quote=FALSE,sep="\t",na="0",row.names=FALSE,col.names=TRUE)
...
forPython <- forPython[order(forPython[,"transcript_cluster_id"]),]
write.table(forPython,file="Results/full.mps",quote=FALSE,sep="\t",na="0",row.names=FALSE,col.names=TRUE)
